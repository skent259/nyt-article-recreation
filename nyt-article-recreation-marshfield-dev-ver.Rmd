...
plot_data <- 
  ...
  arrange(date) %>% 
  mutate(cum.max.cases = cummax(cases),
         cases = ifelse(cases < cum.max.cases, cum.max.cases, cases)) %>% 
...
\begin{aligned}
  ...
  \frac{c_1}{c_0} &= (\Delta_t+1)^{`r window`} \\
  x &= \frac{`r window`\log_{10}(2)}{\log_{10}((\Delta_t+1)^{`r window`})} \\
  &= \frac{\log_{10}(2)}{\log_{10}(\Delta_t+1)} \\
\end{aligned}
$$

where $c$ is the number of cases at time $t$, $c_0$ is the number of cases at time $t_0=t-`r window`$, and $\Delta_t+1$ is the average daily change.
