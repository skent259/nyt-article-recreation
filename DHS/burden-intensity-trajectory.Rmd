...
standard_line_plot <- function(df, x, y, color) {
  x_f <- as.formula(paste0("~", x))
  y_f <- as.formula(paste0("~", y))
  color_f <- as.formula(paste0("~", color))
  color_pal <- "Spectral"
  plot_symbol <- I(23)
  ...
  plot_ly(data = df,
            ...
            colors = color_pal,
            alpha = 0.7,
            text = ~paste0("</br>", eval(parse(text = color)),
...
fig1 <- 
  standard_line_plot(df, "burden", "intensity", "herc_region") %>% 
  standard_layout() %>% 
...
regions <- unique(wi_counties$herc_region)
  
df <- process_data(wi_counties, group_var = county)

for(i in 1:length(regions)) {
  df_region <- filter(df, herc_region == regions[i])
  
  fig2 <- 
    standard_line_plot(df_region, "burden", "intensity", "county") %>% 
    standard_layout() %>% 
    layout(xaxis = list(title = "Burden: New Cases per 100k (last 14 days)"),
           yaxis = list(title = "Intensity: New Cases (last 7 days)")) 
  
  l[[regions[i]]] <- as_widget(fig2)
...
fig1 <- 
  standard_line_plot(df, "burden", "trajectory", "herc_region") %>% 
  standard_layout() %>% 
  layout(xaxis = list(title = "Burden: New Cases per 100k (last 14 days)"),
...
for(i in 1:length(regions)) {
  ...
  fig2 <- 
    standard_line_plot(df_region, "burden", "trajectory", "county") %>% 
    standard_layout() %>% 
    layout(xaxis = list(title = "Burden: New Cases per 100k (last 14 days)"),
           yaxis = list(title = "Trajectory: Percent Case Change (last 2 weeks)",
                        tickformat = "%"))
...
___Click on link for animation over last 4 weeks___: https://data-viz.it.wisc.edu/dhs-activity-level-county-map-anim/
...
fig_counties_activity

```

<!-- ```{r} -->
<!-- subplot(fig_regions_activity, fig_counties_activity) %>% -->
<!--   # animation_opts(frame = 1000, transition = 0, redraw = FALSE, mode = "afterall") %>% -->
<!--   # animation_slider() %>% -->
<!--   config(displaylogo = FALSE) -->
...
data_frame(`Burden Status` = c("Low", "Moderate", "Moderately High", "High"),
           `Value (per 100,000 Wisconsin residents in the past two weeks)` = c("Case rate $\\leq 10$", "$10 <$ Case rate $\\leq 50$","$50 <$ Case rate $\\leq 100$","Case rate $> 50$")) %>% 
...
data_frame(`Trajectory Status` = c("Shrinking", "Growing", "No Significant Change"),
  ...
  mutate_at(vars(`Trajectory Status`),
            ~cell_spec(.x,
                       background = case_when(.x == "Shrinking" ~ "#4575b4",
                                              .x == "Growing" ~ "#fee090",
                                              .x == "No Significant Change" ~ "#d73027",
                                              TRUE ~ "#cccccc"))) %>%
  kable(escape = F) %>% 
  kable_styling(full_width = F)
...
data_frame(`Burden Status` = c("Low", "Moderate", "Moderately High", "High"),
  ...
  mutate_at(vars(`Shrinking`, `No Significant Change`, `Growing`),
            ~cell_spec(.x,
                       background = case_when(.x == "Low" ~ "#4575b4",
                                              .x == "Medium" ~ "#fee090",
                                              .x == "High" ~ "#d73027",
                                              TRUE ~ "#cccccc"))) %>%
